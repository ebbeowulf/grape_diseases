...
class MultiSpectralDetection(Dataset):
    ...
    def _load_boxes(self, path):
        ...
        Roboflow annotations. It is carried through so SAM3 proposals can be
        ranked for NMS; an entry that is a bare box gets 1.0. This is the
        only place the annotation format is assumed."""
        ...
        for name, entries in data.items():
            for entry in entries:
                if isinstance(entry, (tuple, list)) and len(entry) == 2:
                    score, box = float(entry[0]), entry[1]
                else:
                    score, box = 1.0, entry
    ...
    def __getitem__(self, index):
        ...
        boxes, names, scores = self._load_boxes(box_path)
        ...
        scores = np.array(keep_scores, np.float32)
        ...
        xyxy, cxcywh, kept, kept_scores = [], [], [], []
        for (bx0, by0, bx1, by1), label, score in zip(boxes, labels, scores):
